...
// Create live stream → return streamer token
router.post('/create', async (req, res) => {
  try {
    const { title, userId } = req.body;
    const roomName = `stream_${Date.now()}`; // unique room ID
    ...
    res.status(500).json({ msg: 'Error creating live' });
  }
});

// Viewer joins → return viewer token
router.post('/:roomName/join', async (req, res) => {
  try {
    ...
    const token = new AccessToken(LIVEKIT_API_KEY, LIVEKIT_API_SECRET, {
      identity: identity || `guest_${Math.random().toString(36).substring(2, 7)}`,
    });
    token.addGrant({
      roomJoin: true,
      room: roomName,
    ...
    res.status(500).json({ msg: 'Error joining live' });
  }
});
