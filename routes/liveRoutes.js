...
router.post('/create', authMiddleware, async (req, res) => {
  try {
    ...
    // Create new livestream document
    const liveStream = new LiveStream({
      title: title.trim(),
      description: description?.trim() || '',
      streamer: req.userId,
    ...
    try {
      // Generate Stream.io details
      const streamDetails = await generateStreamDetails(liveStream._id.toString(), req.userId);
      console.log('Stream.io details:', streamDetails);

      // Update the stream with Stream.io details
      liveStream.streams.push({
        user: req.userId,
        joinedAt: new Date(),
        callId: streamDetails.callId,
        streamType: 'livestream',
      });

      // Store Stream.io details in the document
      liveStream.streamioCallId = streamDetails.callId;
      liveStream.streamioApiKey = streamDetails.apiKey;
      ...
      const responseData = {
        streamId: liveStream._id,
        callId: streamDetails.callId,
        publisherToken: streamDetails.publisherToken,
        viewerToken: streamDetails.viewerToken,
        apiKey: streamDetails.apiKey,
      ...
      console.error('Stream generation error:', streamError);
      // Clean up the created stream if Stream.io setup fails
      await LiveStream.findByIdAndDelete(liveStream._id);
      res.status(500).json({ msg: `Could not create live stream: ${streamError.message}` });
    }
  } catch (error) {
...
router.post('/:streamId/add-cohost', authMiddleware, async (req, res) => {
  try {
    ...
    // Generate cohost token
    const cohostDetails = await addCohost(liveStream.streamioCallId, userId);
    
    liveStream.streams.push({
      user: userId,
      joinedAt: new Date(),
      callId: liveStream.streamioCallId,
      streamType: 'livestream',
    });
    ...
    res.json({
      msg: 'Co-host added',
      stream: liveStream,
      cohostToken: cohostDetails.cohostToken,
      callId: cohostDetails.callId,
      apiKey: cohostDetails.apiKey,
    });
  } catch (error) {
    console.error('Add co-host error:', error);
    res.status(500).json({ msg: 'Could not add co-host' });
...
router.post('/:streamId/end', authMiddleware, async (req, res) => {
  try {
    ...
    // End Stream.io call
    if (liveStream.streamioCallId) {
      await endLiveStream(liveStream.streamioCallId);
    }
...
router.get('/:streamId/token', async (req, res) => {
  try {
    ...
    const viewerToken = await generateViewerToken(liveStream.streamioCallId, viewerId);
    
    res.json({
      viewerToken,
      callId: liveStream.streamioCallId,
      apiKey: liveStream.streamioApiKey,
      streamType: 'livestream',
    });
  } catch (error) {
    console.error('Get viewer token error:', error);
...
router.get('/:streamId', async (req, res) => {
  try {
    ...
    try {
      const viewerToken = await generateViewerToken(liveStream.streamioCallId, viewerId);
      liveStream._doc.viewerToken = viewerToken;
      liveStream._doc.callId = liveStream.streamioCallId;
      liveStream._doc.apiKey = liveStream.streamioApiKey;
      liveStream._doc.streamType = 'livestream';
    } catch (tokenError) {
      console.error('Error generating viewer token:', tokenError);
      // Continue without token - frontend can handle gracefully
    }
...
// Test Stream.io credentials
router.get('/test-credentials', async (req, res) => {
  try {
    const result = await testStreamConnection();
    res.json({
      message: 'Stream.io credential test completed',
      result,
    });
  } catch (error) {
    res.status(500).json({
      error: error.message,
      message: 'Stream.io credential test failed'
    });
  }
});
