...
router.post('/conversations/:conversationId/messages', authMiddleware, async (req, res) => {
    try {
        const { conversationId } = req.params;
        const { content, type = 'text', fileUrl, fileSize, fileName, key } = req.body; // Add key here
        ...
        if (['image', 'video', 'audio', 'file'].includes(type) && !fileUrl) {
            return res.status(400).json({ msg: 'File URL is required for media messages' });
        }
