...
// Stream.io credentials - replace with your actual credentials
const STREAM_API_KEY = "zz596cfeas5g";
const STREAM_API_SECRET = "m6jnyemxarexdkqdr7uf96pp3a3fahtpwkyny5vbkvj7qkr94fvnrfhcrkzgxzkb";
...
export const generateStreamDetails = async (streamId, userId) => {
  try {
    console.log('Creating Stream.io live stream:', { streamId, userId });
    ...
    const publisherToken = streamClient.generateUserToken({
      user_id: userId,
      exp: Math.floor(Date.now() / 1000) + (24 * 60 * 60), // 24 hours
    });

    // Generate viewer token (for anonymous viewers)
    const viewerToken = streamClient.generateUserToken({
      user_id: `viewer_${Date.now()}`,
      role: 'viewer',
      exp: Math.floor(Date.now() / 1000) + (24 * 60 * 60), // 24 hours
    });

    console.log('Stream.io tokens generated successfully');
    ...
    console.error('Stream.io stream creation error:', error);
    throw new Error(`Failed to create Stream.io live stream: ${error.message}`);
  }
};

export const endLiveStream = async (callId) => {
  try {
    const call = streamClient.call('livestream', callId);
    await call.endCall();
    console.log(`Stream.io call ${callId} ended`);
  } catch (error) {
    console.error('Error ending Stream.io call:', error);
  }
};

export const generateViewerToken = async (callId, viewerId) => {
  try {
    const viewerToken = streamClient.generateUserToken({
      user_id: viewerId || `viewer_${Date.now()}`,
      role: 'viewer',
      exp: Math.floor(Date.now() / 1000) + (24 * 60 * 60), // 24 hours
    ...
    console.error('Stream.io viewer token error:', error);
    throw new Error(`Failed to generate viewer token: ${error.message}`);
  }
};

export const addCohost = async (callId, userId) => {
  try {
    // Generate cohost token with publish permissions
    const cohostToken = streamClient.generateUserToken({
      user_id: userId,
      role: 'host',
      exp: Math.floor(Date.now() / 1000) + (24 * 60 * 60), // 24 hours
      call_cids: [`livestream:${callId}`],
    ...
    throw new Error(`Failed to add cohost: ${error.message}`);
  }
};

// Test Stream.io connection
export const testStreamConnection = async () => {
  try {
    console.log('Testing Stream.io connection...');
    
    if (!STREAM_API_KEY || !STREAM_API_SECRET) {
      throw new Error('Missing Stream.io credentials');
    }
    ...
    console.log('Stream.io connection test passed');
    return { success: true, token: !!testToken };
  } catch (error) {
    console.error('Stream.io connection test failed:', error);
